...
def click_ver_detalhes_for_date(driver, date_str: str, wait_sec: int = WAIT_SEC) -> bool:
    ...
    try:
        ...
        xpath_btn = f"//tr[td/div[@align='center' and normalize-space(.)='{date_str}']]//a[img[contains(@src,'bt_ver_detalhes.gif')]]"
        row = W(driver, wait_sec).until(EC.element_to_be_clickable((By.XPATH, xpath_btn)))
    ...
    for s in strong_nodes:
        try:
            ...
            if current_uf and current_municipio:
                norm_txt = _normalize_spaces(txt)

                # Ignorar cabeçalhos óbvios
                if any(key.lower() in norm_txt.lower() for key in blacklist_header_snippets):
                    continue

                # Verifica se contém data
                contains_date = bool(DATE_ANY_RX.search(txt))
                if not contains_date:
                    continue

                # Verifica se está dentro de um link (indicativo de ATO real)
                inside_link = False
                try:
                    s.find_element(By.XPATH, "ancestor::a")
                    inside_link = True
                except Exception:
                    inside_link = False

                if not inside_link:
                    # Se não está dentro de <a>, provavelmente é cabeçalho → ignora
                    continue

                # Agora trata como ATO válido
                ato = txt
